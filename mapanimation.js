angular.module('busTrackerApp', []).controller('BusTrackerCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $http({
    method: 'GET',
    url: 'https://api-v3.mbta.com/routes?api_key=ac149cccb08b4e288b949e58d9eac162'
  }).then(function successCallback(response) {
    // console.log("succeeded! response = ");
    // console.log(response);
    self.routes = response.data.data; 
    console.log("succeeded! self.routes = ");
  ...
  //   setTimeout(() => {
  //     if (self.counter >= self.busStops.length) return;
  //     self.marker.setLngLat(self.busStops[self.counter]);
  //     self.counter++;
  //     self.move();
  //   }, 1000);
  // }
